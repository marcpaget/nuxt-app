const COUNTRIES = ["AFG", "ALA", "ALB", "DZA", "ASM", "AND", "AGO", "AIA",
"ATA", "ATG", "ARG", "ARM", "ABW", "AUS", "AUT", "AZE", "BHS", "BHR", "BGD",
"BRB", "BLR", "BEL", "BLZ", "BEN", "BMU", "BTN", "BOL", "BES", "BIH", "BWA",
...
"VEN", "VNM", "VGB", "VIR", "WLF", "ESH", "YEM", "ZMB", "ZWE"] export default
function randomCountry() { const multipleCountries = [] for (let i = 0; i < 4;
i++) { const random = Math.floor(Math.random() * (COUNTRIES.length - 1))
multipleCountries.push(COUNTRIES[random]) } if
(isAnyCodeRepeat(multipleCountries)) { randomCountry() } return
multipleCountries } function isAnyCodeRepeat (countries) { for (let i = 0; i <
countries.length ; i++) { if (i === countries.length - 1) return false return
countries.includes(countries[i], i+1) }
